...
const CreateTask = () => {
    ...
    const createTask = async () => {
        try {
            ...
            const createdTask = await tasks.createTask(auth.user.roomId, data);
            if (!createdTask) throw Error();
            setState(prev => ({ ...prev, success: "Task created successfully" }))
            setState(prev => ({ ...prev, error: null }));
            setData({ title: '', description: '', priority: Priority.LOW, attachments: [], dueDate: '', checkItems: [], assignTo: [] });
        ...
        <>
            ...
            <div className={`[scrollbar-width:none] [-ms-overflow-style:none] [-webkit-scrollbar-width]:hidden px-5 max-w-250 w-full h-full overflow-y-auto overflow-x-hidden py-8`}>
                <div className={`flex flex-col gap-4 w-full rounded-md border-1 border-gray-200 bg-white px-4 py-5`}>
                    <h1 className={`text-xl font-['PoppinsSemiBold']`}>Create Task</h1>
                    ...
                    <div className={`flex flex-col gap-5`}>
                        ...
                        <div className={`w-full flex gap-4 items-center max-[850px]:flex-col max-[850px]:justify-start`}>
                            <div className={`flex max-[850px]:w-full flex-col gap-3`}>
                    ...
                    <Button onClick={createTask} className={'h-[50px]'}>Create</Button>
                </div>
            </div>
        </>
    )
